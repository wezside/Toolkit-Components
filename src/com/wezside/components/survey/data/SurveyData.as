package com.wezside.components.survey.data
{
	...
	public class SurveyData implements ISurveyData, IDeserializable {
		...
		public function getProgressItemByFormID( id : String ) : ProgressBarItem {
			
			if ( progress && progress.find() ) {
				
				var pbData : ProgressBarData = ProgressBarData( progress.find() );
				
				if ( pbData ) {
					
					var it : IIterator = pbData.items.iterator();
					var pbItem : ProgressBarItem;
					var found : Boolean = false;
					
					while ( it.hasNext() ) {
						pbItem = ProgressBarItem( it.next() );
						if ( pbData.hasFormID( pbItem, id ) ) {
							found = true;
							break;
						}
					}
		...
		public function get component() : ICollection { return _component; }
		public function set component( value : ICollection ) : void { _component = value; }
		
		public function get customCSS() : ICollection { return _customCSS; }
		public function set customCSS( value : ICollection ) : void { _customCSS = value; }
		
		public function get customForm() : ICollection { return _customForm; }
		public function set customForm( value : ICollection ) : void { _customForm = value; }
		
		public function get layout() : ICollection { return _layout; }
		public function set layout( value : ICollection ) : void { _layout = value; }
		
		public function get metaData() : ICollection { return _metaData; }
		public function set metaData( value : ICollection ) : void { _metaData = value; }
		
		public function get progress() : ICollection { return _progress; }
			...
			while ( it.hasNext() )
			{
				...
				while ( itFormGroup.hasNext() )
				{
					...
					while ( formItemIt.hasNext() )
					{
						...
						formItemData.value = ""; // Default Value
					}
					formItemIt.purge();
					formItemIt = null;
					formItemData = null;
